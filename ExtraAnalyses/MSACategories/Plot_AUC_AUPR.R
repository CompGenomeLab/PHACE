...
group_sets <- list(
  "NumSeq" = list(
    ...
    "Medium" = data$ID[data$NumSeq > num_seq[1] & data$NumSeq <= num_seq[2]],
    "Large" = data$ID[data$NumSeq > num_seq[2]]
  ),
  "NumPos" = list(
    "Small" = data$ID[data$NumPos <= num_pos[1]],
    "Medium" = data$ID[data$NumPos > num_pos[1] & data$NumPos <= num_pos[2]],
    "Large" = data$ID[data$NumPos > num_pos[2]]
  ),
  "TreeLen" = list(
    "Small" = data$ID[data$TreeLength <= tree_len[1]],
    "Medium" = data$ID[data$TreeLength > tree_len[1] & data$TreeLength <= tree_len[2]],
    "Large" = data$ID[data$TreeLength > tree_len[2]]
  )
)
...
for (attribute in names(group_sets)) {
  ...
  final_plot <- plot_grid(
    ...
    # Third row with label "C"
    plot_grid(plotlist = plot_list[9:12], ncol = 4),
    ncol = 1,
    labels = c("A", "B", "C"),
    label_size = 16,
    label_x = 0, label_y = 1
